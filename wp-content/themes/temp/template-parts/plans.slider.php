<section class="plans">
    <div class="header">
        <h2>ВыбЕРИте подходящую планировку</h2>
        <hr class="hide-on-mobile">
    </div>
    <drag-scroll class="variants">
        <div class="card">
            <h3>Студия</h3>
            <img alt="Студия" loading="lazy" decoding="async" src="/images/plans/plan.1.svg">
            <p>от
                <offer-data data-group="studio" data-key="price" data-round="1000000"></offer-data>
                млн. рублей
            </p>
            <a href="<?php bloginfo( 'url' ); ?>/plans#studios">ознакомиться подробнее</a>
        </div>
        <div class="card">
            <h3>1-комнатная квартира</h3>
            <img alt="1-комнатная квартира" loading="lazy" decoding="async" src="/images/plans/plan.2.1.svg">
            <p>от
                <offer-data data-group="1" data-key="price" data-round="1000000"></offer-data>
                млн. рублей
            </p>
            <a href="<?php bloginfo( 'url' ); ?>/plans#1rooms">ознакомиться подробнее</a>
        </div>
        <div class="card">
            <h3>2-комнатная квартира</h3>
            <img alt="2-комнатная квартира" loading="lazy" decoding="async" src="/images/plans/plan.3.2.svg">
            <p>от
                <offer-data data-group="2" data-key="price" data-round="1000000"></offer-data>
                млн. рублей
            </p>
            <a href="<?php bloginfo( 'url' ); ?>/plans#2rooms">ознакомиться подробнее</a>
        </div>
        <div class="card">
            <h3>3-комнатная квартира</h3>
            <img alt="3-комнатная квартира" loading="lazy" decoding="async" src="/images/plans/plan.4.svg">
            <p>от
                <offer-data data-group="3" data-key="price" data-round="1000000"></offer-data>
                млн. рублей
            </p>
            <a href="<?php bloginfo( 'url' ); ?>/plans#3rooms">ознакомиться подробнее</a>
        </div>
        <div class="card">
            <h3>4-комнатная квартира</h3>
            <img alt="4-комнатная квартира" loading="lazy" decoding="async" src="/images/plans/plan.5.svg">
            <p>от
                <offer-data data-group="4" data-key="price" data-round="1000000"></offer-data>
                млн. рублей
            </p>
            <a href="<?php bloginfo( 'url' ); ?>/plans#4rooms">ознакомиться подробнее</a>
        </div>
        <button class="scroll-button" data-scroll="prev"></button>
        <button class="scroll-button" data-scroll="next"></button>
    </drag-scroll>
</section>